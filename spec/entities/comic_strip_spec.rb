...
describe ComicStrip do
	describe '.ocr' do
		...
		before { ComicStrip.should_receive(:recognize_bubbles).with(bw_image).and_return(bubbles) }
	...
	describe '.recognize_bubbles' do
		def image_from_string(s)
			BWImage.new(s)
		end
		context '1x1 black image' do
			subject { ComicStrip.recognize_bubbles(image_from_string("x")) }
			it { should == [] }
		end
		context '1x1 white image' do
			subject { ComicStrip.recognize_bubbles(image_from_string(".")) }
			its(:count) { should == 1 }
